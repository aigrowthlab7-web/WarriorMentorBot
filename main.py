...
async def relapse_command(update, context):
    user_id = update.effective_user.id
    user_streaks[user_id] = 0
...
def main():
    if not GROQ_API_KEY:
        print("ERROR: GROQ_API_KEY missing in Secrets!")
        return
...
if __name__ == "__main__":
    main()
~/workspace$ ^C
~/workspace$ cat > requirements.txt << 'EOF'
python-telegram-bot==22.6
groq==1.0.0
EOF
echo "Requirements file ready!"
Requirements file ready!
~/workspace$ ^[[200~cat main.py~
cat main.py~bash: cat: command not found
~/workspace$ cat main.py
import os
from telegram import Update
from telegram.ext import Application, CommandHandler, MessageHandler, filters, ContextTypes
from groq import Groq
